...
class RocketScienceDataset:
    ...
    def from_hub(
        ...
        **kwargs,
        ...
        choose one; only that prefix is downloaded. `subdir` overrides the prefix for a non-split
        layout. Requires the `hf` extra.

        Pass `shards=N` to download only the first `N` tar shards (plus the index) and restrict the
        dataset to the matches they contain -- handy for a quick smoke test without pulling the full
        split (e.g. `shards=1` fetches a single shard). `None` (default) downloads every shard.
        ...
        if shards is None:
            # fnmatch-based, so `{prefix}/*` also matches shards nested in subfolders
            local = snapshot_download(
                repo_id, repo_type="dataset", revision=revision, allow_patterns=[f"{prefix}/*"]
            )
            return cls.from_local(Path(local) / prefix, **kwargs)

        if shards < 1:
            raise ValueError(f"shards must be >= 1 (or None for all), got {shards}")
        # Fetch the index first, pick the first `shards` tar files, then download only those.
        index_local = snapshot_download(
            repo_id, repo_type="dataset", revision=revision, allow_patterns=[f"{prefix}/index.json"]
        )
        index = Index.load(Path(index_local) / prefix / "index.json")
        keep_shards = sorted({e.shard for e in index.entries})[:shards]
        local = snapshot_download(
            repo_id,
            repo_type="dataset",
            revision=revision,
            allow_patterns=[f"{prefix}/index.json", *(f"{prefix}/{s}" for s in keep_shards)],
        )
